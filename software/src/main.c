...
void _wtimer0_isr()
{
    ...
    if(ulFlags & WTIMER_IF_OF)
    {
        ...
        WTIMER1->CC[1].CCV = (PHASE_ANGLE_WIDTH - g_usPacLookup[(uint16_t)ovenPid.fOutput]) / 0.028f;
...
int main()
{
   ...
   // PID initialization
   ovenPid = pid_init(0, PHASE_ANGLE_WIDTH, PID_KP, PID_KD, PID_KI);
   ...
   // Wide Timer 0 - Capture zero cross
    ...
    while(1)
    {
        ...
        if(mcp9600_get_status() &  MCP9600_TH_UPDT)
        {
            ...
            DBGPRINTLN_CTX("PID - output %f / %d", ovenPid.fOutput, PHASE_ANGLE_WIDTH);
            DBGPRINTLN_CTX("PID - output linear compensated %d / %d", g_usPacLookup[(uint16_t)ovenPid.fOutput], PHASE_ANGLE_WIDTH);
        }
/*

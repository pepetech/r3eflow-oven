    ...
    if(flags & TIMER_IF_OF)
    {
        if(soundQueueHead)
        {
            if(soundQueueHead->frequency)
            {
                ...
                TIMER0->TOP = (HFPER_CLOCK_FREQ / (soundQueueHead->frequency << 1)) - 1; // Double the frequency
            ...
            TIMER2->TOP = (soundQueueHead->duration * 35U);
...
void sound_play(uint16_t frequency, uint32_t duration)
{
    ...
    TIMER0->TOP = (HFPER_CLOCK_FREQ / (frequency << 1)) - 1; // Double the frequency
    TIMER0->CMD = TIMER_CMD_START;

    TIMER2->TOP = (duration * 35U);
    TIMER2->CMD = TIMER_CMD_START;
}

void sound_queue_add(uint16_t frequency, uint32_t duration)
{
    sound_queue_element_t* element = malloc(sizeof(sound_queue_element_t));
